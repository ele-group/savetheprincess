...
$e = $event ?: $e; // legacy code from 16 rc-2
		...
		$instance = $this->create('LevelThreeBackground.shot_ufo_small', $this->image21);
		$this->instances('LevelThreeBackground.shot_ufo_small')->x = 2800;
		$this->instances('LevelThreeBackground.shot_ufo_small')->y = -35;
		Score::inc('enemy1hp', -17);

		if (Score::get('enemy1hp') != -2)
			return;
		$e->sender->sprite->currentAnimation = '4';
...
$e = $event ?: $e; // legacy code from 16 rc-2
		...
		Animation::fadeIn($this->object_ufo_small_2, 500, function () use ($e, $event) {
		});
...
$e = $event ?: $e; // legacy code from 16 rc-2
		...
		$this->instances('LevelThreeBackground.shot_ufo_small')->x = 2800;
		$this->instances('LevelThreeBackground.shot_ufo_small')->y = -35;
		Score::inc('enemy2hp', -17);

		if (Score::get('enemy2hp') != -2)
			return;
		$e->sender->sprite->currentAnimation = '4';
		waitAsync(500, function () use ($instance, $e, $event) {
			Animation::fadeOut($e->sender, 500, function () use ($instance, $e, $event) {
...
$e = $event ?: $e; // legacy code from 16 rc-2
		...
		Animation::fadeOut($this->target, 500, function () use ($e, $event) {
			Animation::fadeOut($this->bow, 500, function () use ($e, $event) {
				Animation::fadeIn($this->sword, 500, function () use ($e, $event) {
					$this->player->sprite->currentAnimation = 'default';
				});
			});
		});
...
$e = $event ?: $e; // legacy code from 16 rc-2
		...
		$e->sender->sprite->currentAnimation = '2';
		waitAsync(250, function () use ($e, $event) {
			Animation::fadeOut($e->sender, 250, function () use ($e, $event) {
				$e->sender->free();
			});
		});
...
$e = $event ?: $e; // legacy code from 16 rc-2
		...
		waitAsync(1200, function () use ($e, $event) {
			$this->instances('LevelThreeBackground.shot_ufo_small')->free();
		});
...
$e = $event ?: $e; // legacy code from 16 rc-2
		...
		$e->sender->free();
		Animation::fadeOut($this->object_ufo_big_1, 500, function () use ($e, $event) {
			Animation::fadeIn($this->object_ufo_big_2, 500, function () use ($e, $event) {
				$this->object_ufo_big_2->sprite->currentAnimation = '1';
				waitAsync(500, function () use ($e, $event) {
					$this->object_ufo_big_2->sprite->currentAnimation = 'default';
				});
			});
		});
...
$e = $event ?: $e; // legacy code from 16 rc-2
		...
		$e->sender->gridMove->disable();
		$e->sender->x = 2208;
		$e->sender->y = 432;
		Animation::fadeOut($this->sword, 500, function () use ($e, $event) {
			Animation::fadeIn($this->bow, 500, function () use ($e, $event) {
				Animation::fadeIn($this->target, 500, function () use ($e, $event) {
					$this->rect_flag_bow->free();
					waitAsync(100, function () use ($e, $event) {
						$e->sender->gridMove->enable();
						$this->bow->gridMove->enable();
					});
				});
			});
		});
...
$e = $event ?: $e; // legacy code from 16 rc-2
		...
		$this->instances('LevelThreeBackground.shot_ufo_small')->free();
		Score::inc('hp', -5);
		Element::setText($this->hp, \action\Score::get('hp'));
...
$e = $event ?: $e; // legacy code from 16 rc-2
		...
		$this->instances('LevelThreeBackground.shot_ufo_1')->free();
...
$e = $event ?: $e; // legacy code from 16 rc-2
		...
		$this->instances('LevelThreeBackground.shot_ufo_2')->free();
...
$e = $event ?: $e; // legacy code from 16 rc-2
		...
		$this->instances('LevelThreeBackground.shot_ufo_3')->free();
...
$e = $event ?: $e; // legacy code from 16 rc-2
		...
		$this->instances('LevelThreeBackground.shot_ufo_4')->free();
